const POPULAR = [
  ...
  function makeName(term, ext) {
    ...
    const clean = term.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
  ...
  function render(term) {
    ...
    document.getElementById('popularList').innerHTML = POPULAR.map(d => renderItem(d, makeName(t, d.ext))).join('');
    document.getElementById('allList').innerHTML    = ALL.map(d => renderItem(d, makeName(t, d.ext))).join('');
  }
  
  const inp = document.getElementById('sInput');
  inp.addEventListener('input',   () => render(inp.value));
  inp.addEventListener('keydown', e  => { if (e.key === 'Enter') render(inp.value); });
  document.getElementById('sBtn').addEventListener('click', () => render(inp.value));
  ...
  FAQS.forEach(item => {
    ...
    el.querySelector('.fq').addEventListener('click', () => {
      document.querySelectorAll('.fi.open').forEach(x => { if (x !== el) x.classList.remove('open'); });
